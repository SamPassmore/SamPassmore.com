...
For the most part, I have been using [Gaussian Process models](https://www.youtube.com/watch?v=PIuqxOBJqLU), and have been building upwards from the [BRMS phylogenetic multilevel model example provided by Paul Bürkner]((https://cran.r-project.org/web/packages/brms/vignettes/brms_phylogenetics.html)).

I have shifted to using a different software package, [INLA](https://www.r-inla.org/), and to ensure I understand the dynamics of this new modelling environment, I replicated Paul Bürkner's example in INLA with the advice and guidance of Russell Dinnage.

To remind us of the premise I copy Paul's introduction below:
...
You may notice that the precision matrix we built contains many more columns and rows than we have taxa. This is because of the setting `nodes = "ALL"`, which builds the precision matrix for all taxa AND nodes. We could change this setting to  `nodes = "TIPS"`, which would mean we have the same number of rows and columns as taxa. Building the precision matrix for both tips and nodes creates a _sparse_ matrix, which makes the model estimation more computationally efficient - even though it includes information we won't use.

This means that unlike BRMS, the rows of the phylogenetic matrix do not line up with the rows of the dataset we are going to model. To accomodate this, we need to match the rows in to the rows in `data_simple` to the rows in `phylo_prec_mat`.

```{r}
phylo_id <- match(phylo$tip.label, rownames(phylo_prec_mat))
```

With the precision matrix built and data lined up we can build the model using the deafult prior settings for INLA.
