...
The most common form of autocorrelation is time. If I am running, a good predictor of my speed at any point is the speed I was going moments earlier. In linguistics and biology, the time element is drawn out over thousands (linguistics) or millions (biology) of years, and we can use phylogenies to track how similar or different langauges or species are and use that information to make inferences on the past (or perhaps future). 

For the most part, I have been using [Gaussian Process models](https://www.youtube.com/watch?v=PIuqxOBJqLU) to quantify autocorrelational effects, and have been building upwards from the [BRMS phylogenetic multilevel model example provided by Paul Bürkner]((https://cran.r-project.org/web/packages/brms/vignettes/brms_phylogenetics.html)). This example shows us how to estimate the amount of phylogenetic signal, specifically Pagel's Lambda, within a Bayesian multilevel model. Paul offers a number of different situations, but I am particularly interested in the simple and repeated-measures model.

While BRMS is an excellent package, an alternative I have started using is [INLA](https://www.r-inla.org/), under the guidance of [Russell Dinnage](https://twitter.com/ecologician). To ensure I understand the dynamics of this new modelling environment, Russell helped me replicated Paul Bürkner's example in INLA.
...
INLA also requires that we identify the relationship between the precision matrix and dataframe using numerical indices, rather than character names as BRMS does. To get indicies I use the `match` function. 
...
```{r}
round(sqrt(1/model_simple_inla$summary.hyperpar), 2)
```
